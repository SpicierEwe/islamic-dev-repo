...
export default function DynamicPage(props) {
  ...
  const DownloadableCtx = useContext(DataContext);

  useEffect(() => {
    // globalising data here
    DownloadableCtx.globaliseDownloadableData(props.responseData), [];
  });
...
async function searchQuery(q1, q2) {
  ...
  const query1 = await q1.toString().toLowerCase();
  const query2 = await q2.toString().toLowerCase();
