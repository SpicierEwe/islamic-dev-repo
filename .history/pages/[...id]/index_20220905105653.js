  ...
  const hostName =
    process.env.NODE_ENV == "development"
      ...
      : // : `${process.env.VERCEL_URL}`;
        `${"http://islamic-dev-repo.vercel.app"}`;
  const x = await fetch(`${hostName}/api/generate_static_paths`).json();
  // console.log(x["paths"]);
...
export async function getStaticProps({ params }) {
  ...
  const responseData = await (await fetch(`/api/${query1}/${query2}`)).json();
