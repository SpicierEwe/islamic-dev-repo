export default async function handler(req, res) {
  const url_template = await JSON.parse(req.body).url_template;
  const reciter_name = await JSON.parse(req.body).reciter_name;
  //
  // console.log(req.body);
  //
  const generated_urls_array = [];
  var totalVerses = 0;
  for (var surahIndex = 0; surahIndex < quranMedaData.length; surahIndex++) {
    for (
      var verse_number = 0;
      verse_number < quranMedaData[surahIndex][1];
      verse_number++
    ) {
      totalVerses += 1;
  ...
  //   console.log(generated_urls_array);
  return res.status(200).json(JSON.stringify(generated_urls_array));
}
