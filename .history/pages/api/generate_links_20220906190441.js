export default async function handler(req, res) {
  ...
  //
  const generateLinks = [];
  coonst totalVerses = 0;
  for (var x = 0; x < quranMedaData.length; x++) {
    for (
      var verse_number = 0;
      verse_number < quranMedaData[x][1];
      verse_number++
    ) {
      totalVerses += 1;
      generateLinks.push({
        ...
        audio_url: `${url_template}${(x + 1).toString().padStart(3, 0)}_${(
          verse_number + 1
        )
          .toString()
          .padStart(3, 0)}`,
  ...
  res.status(200).json({ generateLinks });
}
