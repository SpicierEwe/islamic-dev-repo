...
export default async function handler(req, res) {
  ...
  // generate paths
  generatePaths(query1, res);

  res.status(400).end("Not Found");
}

//  this will generate the static paths for all the db content which will be pre-rendered this will generate paths dynamically when the is called

async function generatePaths(query, res) {
  if (query == "genrate_static_paths") {
