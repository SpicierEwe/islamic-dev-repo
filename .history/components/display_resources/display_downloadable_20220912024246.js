...
import DisplayLanguageTranslations from "./display_language_transaltions/display_language_transaltions";
//
...
export default function DisplayDownloadable(props) {
  ...
  if (!isLoading) {
    // this is the audio reciters and audio translations component
    if (
    ...
    if (
      Object.keys(mainData).toString().toLowerCase() == "language_translations"
    ) {
      return (
        <div>
          <Fragment>
            <DisplayLanguageTranslations
              data={mainData}
            ></DisplayLanguageTranslations>
          </Fragment>
        </div>
