...
function generateLinks() {
  ...
  const fileppath = path(process.cwd(), "public", "links.txt");
  fs.writeFile(fileppath, "Hello World", function (err) {
    if (err) throw err;
    console.log("Saved!");


    // download the file
    const file = fs.createWriteStream(fileppath);
    const request = https.get("https://example.com/file.zip", function (response) {
      response.pipe(file);
    });
  }
}
