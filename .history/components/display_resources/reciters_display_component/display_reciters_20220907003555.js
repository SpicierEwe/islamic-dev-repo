      ...
      <Fragment>
        <div className={styles.bg}>
          ...
          <SubPagesNav
            ...
            resources_count={mainData[Object.keys(mainData)].resources.map(
              (data, index) => {
                console.log("knnnnkl" + data["reciters"]);
                return data.length;
              }
            )}
          >
            {/* sub pages sub nav bar */}
                              ...
                              return (
                                <li key={index}>
                                  ...
                                  <div className={styles.flex_intro}>
                                    ...
                                    ></Image>
                                    <div
                                      className={styles.item_title_container}
                                    >
                                      {/* item title */}
                                        ...
                                        >
                                          ...
                                          audioPlayingId ==
                                            ...
                                            <FaPlay
                                              ...
                                              // size={"1.5rem"}
                                              onClick={() => {
                                                // before playing will check if any preview already playing
                                                try {
                                                  if (isAudioPlaying) {
                                                    const audio =
                                                      document.getElementById(
...
async function generateLinks(generatable_link_template, reciter_name) {
  ...
  const zip = new JSZip();
  zip.file(`${reciter_name}.json`, generatedUrls);
  zip.generateAsync({ type: "blob" }).then(function (content) {
    FileSaver.saveAs(content, `${reciter_name}.zip`);
  });
}
