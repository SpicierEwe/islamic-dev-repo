...
export default function DisplayDownloadable(props) {
  ...
  function languageTranslationsDownloadButton(data, index) {
    const download = async (url) => {
      return await fetch(url, { method: "GET", mode: "no-cors" }).then((resp) =>
        resp.blob()
      );
    };
    ...
    const downloadAndZip = (urls) => {
      return downloadMany(urls).then((value) => exportZip(value));
    };
    ...
    return (
      <a
        ...
        onClick={() => {
          ...
          var urls = [
            "https://tanzil.net/trans/?transID=sq.nahi&type=txt",
