    ...
    return (
      <a
        ...
        onClick={async () => {
          ...
          // var x = await fetch("/api/tt");
          // var x = new Blob(["hiiii jjbcbsddnfdfk "], { type: "text/plain" });

          async function download(bytes, filename) {
            let elm = document.createElement("a"); // CREATE A LINK ELEMENT IN DOM
            elm.href = bytes; // SET LINK ELEMENTS CONTENTS
            elm.setAttribute("download", filename); // SET ELEMENT CREATED 'ATTRIBUTE' TO DOWNLOAD, FILENAME PARAM AUTOMATICALLY
            elm.click(); // TRIGGER ELEMENT TO DOWNLOAD
          }

          // download("https://tanzil.net/trans/bn.hoque", "./output.zip");

          var data = fetch("/api/tt").then((res) => res.arrayBuffer);
          console.log(data);
          download(data, "aa.zip");
        }}
      >
        <Image
          onMouseEnter={() => {
